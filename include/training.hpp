...
template <typename T> /* Sublcass of spin system. */
struct training_step_result training_step_parallel(NeuralState &nqstate, T spin_sys, 
    int nsweeps, float gamma, int rank, int world_size
) {
    SamplerResult sampler_result;
    // Norms of the total estimated gradients.
    float norm_weight_grad;
    ...
    sampler_result = sample_spins_parallel<T>(nqstate, spin_sys, nsweeps, rank, world_size);
